...
while($row=mysqli_fetch_array($result)){
	if($last_brand!=$row['Brand']){
		...
		/*if(isset($_GET['filter_brands']) && $_GET['sort_products']){
			...
			$query="SELECT Brand FROM products WHERE Type='$product_type' AND Brand='$filter_brands' ORDER BY $sort_products";
			if($filter_brands=="All"){
				$query="SELECT Brand FROM products WHERE Type='$product_type' ORDER BY $sort_products";
			}
		}else{
			$query="SELECT Brand FROM products WHERE Type='$product_type' ORDER BY ID DESC";
		}*/
